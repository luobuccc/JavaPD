...
public class Main {
    ...
    public static BigDecimal sqrt(BigDecimal number, int scale, int roundingMode) {
        if (number.compareTo(BigDecimal.ZERO) < 0) throw new ArithmeticException("sqrt with negative");
    ...
    public static BigDecimal sqrt(BigDecimal number, int scale) {
        return sqrt(number, scale, BigDecimal.ROUND_DOWN);
    }
    ...
    public static void main(String args[]) {
        ...
        while (in.hasNext()) {
            ...
            BigDecimal root = sqrt(num, 200, 1);
            System.out.printf("%s\n", root);
        }
